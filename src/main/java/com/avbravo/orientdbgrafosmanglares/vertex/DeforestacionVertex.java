...
public class DeforestacionVertex {
    public void createSchema(ODatabaseSession db) {
        try {
             ...
             *
             */
            OClass sombra = db.getClass("Deforestacion");

            if (sombra == null) {
                sombra = db.createVertexClass("Deforestacion");

            }

            if (sombra.getProperty("name") == null) {
                sombra.createProperty("name", OType.STRING);
                sombra.createIndex("Deforestacion_name_index", OClass.INDEX_TYPE.NOTUNIQUE, "name");
            }
      ...
      public OVertex insert(ODatabaseSession db, String name) {
        OVertex result = db.newVertex("Deforestacion");
        try {
